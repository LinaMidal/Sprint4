...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	meanSpeedKmInH := meanSpeed(action, duration)
	runSpendCalories := (runningCaloriesMeanSpeedMultiplier * meanSpeedKmInH * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
	return runSpendCalories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	multiplierFirst := walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedMInSec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight
	multiplierSecond := duration * minInH
	walkSpendCalories := multiplierFirst * multiplierSecond
	return walkSpendCalories
}
...
func swimmingMeanSpeed(lengthPool, countPool int, duration float64) float64 {
	...
	return float64(lengthPool) * float64(countPool) / mInKm / duration //для меня: измеряется в км/час
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	meanSpeedKmInH := swimmingMeanSpeed(lengthPool, countPool, duration)
	swimSpendCalories := (meanSpeedKmInH + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimSpendCalories
}
